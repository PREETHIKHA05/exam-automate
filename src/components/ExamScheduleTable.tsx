            ...
            <thead className="bg-gray-50">
              <tr>
                ...
                {departments.map(dept => (
                  <th key={dept.code} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {dept.code}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {uniqueDates.map(date => (
                <tr key={date} className="hover:bg-gray-50">
                  ...
                  {departments.map(dept => {
                    const exam = examsByDateAndDept[date][dept.code];
                    return (
                      <td key={dept.code} className="px-6 py-4 text-sm text-gray-900">
                        {exam ? (
